...
const LeanPaySetup = () => {
  ...
  const fetchBeneficiaries = async () => {
    ...
    try {
      ...
      setInfo("Beneficiaries loaded successfully.");
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  const createPaymentIntent = async () => {
    if (!selectedBeneficiary || !paymentAmount || !paymentCurrency) {
      setError("Please select beneficiary, amount, and currency");
      return;
    }
    ...
    try {
      const paymentData = {
        ...
        payment_destination_id: selectedBeneficiary,
        user_id: parseInt(userRegistered.id),
        description: paymentDescription || `Payment to ${selectedBeneficiary}`,
      };
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Payment intent creation failed: ${res.status} ${errorText}`
        );
      }
      ...
      setInfo("Payment intent created successfully!");
      setPaymentSuccess(false);
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  const executePayment = async () => {
    if (!paymentIntent) {
      setError("Please create a payment intent first");
      return;
    }

    if (!window.Lean || typeof window.Lean.pay !== "function") {
      setError("Lean SDK not loaded or pay() unavailable");
      return;
    }

    try {
      setInfo("Executing payment via Lean SDK...");

      window.Lean.pay({
        access_token: leanTokens?.leanAccessToken,
        app_token: APP_TOKEN,
      ...
      setError(`Payment execution failed: ${err.message}`);
    }
  };
  ...
  // Load beneficiaries when user is connected
  useEffect(() => {
    if (connected) {
      fetchBeneficiaries();
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-800 mb-2">
            Financial Data Integration & Payments
          </h1>
          <p className="text-gray-600">
            Connect your bank accounts securely using Lean API and make payments
          </p>
        </div>
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 p-4 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="text-gray-600">
                    Ready to connect your financial data and make payments
                  </p>
                </div>
                <button
                  onClick={resetAll}
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex space-x-4">
                        ...
                        <button
                          ...
                          disabled={!paymentIntent || loadingPayment}
                          className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-semibold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          Execute Payment
                        </button>
                      </div>
                      ...
                      <div className="bg-gray-50 px-4 py-3 border-b">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                          ...
                          <div className="flex space-x-2">
                            <button
                              onClick={() =>
                                fetchAccountBalance(selectedAccount.account_id)
                              }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div className="flex items-end">
                            <button
                              onClick={handleFetchTransactions}
                              disabled={loadingTransactions}
                              className="w-full bg-green-600 text-white py-2 rounded-lg hover:bg-green-700 font-semibold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              Apply Filter
                            </button>
                          </div>
